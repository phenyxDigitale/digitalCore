...
class TopMenuColumn extends PhenyxObjectModel {
    ...
    public static $definition = [
        ...
        'fields'    => [
            ...
            'img_value_over'          => ['type' => self::TYPE_HTML, 'lang' => true, 'validate' => 'isString'],
            'value_over'              => ['type' => self::TYPE_HTML, 'lang' => true, 'validate' => 'isString'],
            'value_under'             => ['type' => self::TYPE_HTML, 'lang' => true, 'validate' => 'isString'],
    ...
    public function getOutpuName() {
        ...
        $hookname = Hook::exec('displayTopMenuColumnOutPutName', ['type' => $this->type, 'menu' => $this], null, true);
    ...
    public function getBackOutputNameValue() {
        ...
        $hookname = Hook::exec('displayTopMenuColumnBackOutputNameValue', ['type' => $this->type, 'menu' => $this], null, true);
        if(is_array($hookname)) {
            $hookname = array_shift($hookname);
            if(!empty($hookname)) {
                $return = $hookname;    
    ...
    public function getFrontOutputValue() {
        ...
        $hookname = Hook::exec('displayTopMenuColumnFrontOutputValue', ['type' => $this->type, 'menu' => $this], null, true);
        if(is_array($hookname)) {
    ...
    public static function getMenuColumsByIdMenu($id_menu, $id_lang, $active = true, $groupRestrict = false) {
        ...
        $query->groupBy('atmc.`id_topmenu_column`');
        $query->orderBy('atmc.`position`');
        $sql = Hook::exec('actiongetMenuColumnsById', ['id_menu' => $id_menu, 'id_lang' => $id_lang, 'active' => $active], null, true);
        if(is_array($sql)) {
            $sql = array_shift($sql);
            if(!empty($sql)) {
                $query = $sql;    
