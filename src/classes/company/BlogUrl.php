 ...
 * @since 1.9.1.0
 */
class BlogUrl extends PhenyxObjectModel {
    ...
    public function setMain() {
        ...
        $res = Db::getInstance()->update('blog_url', ['main' => 0], 'id_company = ' . (int) $this->id_company);
        $res &= Db::getInstance()->update('blog_url', ['main' => 1], 'id_blog_url = ' . (int) $this->id);
        $this->main = true;

        $sql = 'SELECT s1.id_blog_url FROM ' . _DB_PREFIX_ . 'blog_url s1
                WHERE (
                    SELECT COUNT(*) FROM ' . _DB_PREFIX_ . 'blog_url s2
                    WHERE s2.main = 1
        ...
        foreach (Db::getInstance()->executeS($sql) as $row) {
            Db::getInstance()->update('blog_url', ['main' => 1], 'id_blog_url = ' . $row['id_blog_url']);
        }
        ...
        if (defined('_EPH_ROOT_DIR_')) {
            return Db::getInstance(_EPH_USE_SQL_SLAVE_)->getValue(
                (new DbQuery())
                    ...
                    ->where($this->id ? '`id_blog_url` != ' . (int) $this->id : '')
            );
        } else {
            return Db::getInstance(_EPH_USE_SQL_SLAVE_)->getValue(
                (new DbQuery())
