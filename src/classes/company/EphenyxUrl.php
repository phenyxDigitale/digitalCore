...
class EphenyxUrl extends PhenyxObjectModel {
    ...
    public function setMain() {
        ...
        $res = Db::getInstance()->update('ephenyx_url', ['main' => 0], 'id_company = ' . (int) $this->id_company);
        $res &= Db::getInstance()->update('ephenyx_url', ['main' => 1], 'id_ephenyx_url = ' . (int) $this->id);
        $this->main = true;

        $sql = 'SELECT s1.id_ephenyx_url FROM ' . _DB_PREFIX_ . 'ephenyx_url s1
                WHERE (
                    SELECT COUNT(*) FROM ' . _DB_PREFIX_ . 'ephenyx_url s2
                    WHERE s2.main = 1
        ...
        foreach (Db::getInstance()->executeS($sql) as $row) {
            Db::getInstance()->update('ephenyx_url', ['main' => 1], 'id_ephenyx_url = ' . $row['id_ephenyx_url']);
        }
